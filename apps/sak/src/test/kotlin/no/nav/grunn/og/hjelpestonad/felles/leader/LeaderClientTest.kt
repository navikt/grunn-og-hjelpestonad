package no.nav.grunn.og.hjelpestonad.felles.leader
...
import no.nav.familie.leader.LeaderEnvironment
...
class LeaderClientTest {
    ...
    fun `Skal returnere null hvis environement variable ELECTOR_PATH ikke eksisterer`() {
        mockkStatic(LeaderEnvironment::class)
        every { LeaderEnvironment.hentLeaderSystemEnv() } returns null
    ...
    fun `Skal returnere true hvis pod er leader`() {
        mockkStatic(LeaderEnvironment::class)
        every { LeaderEnvironment.hentLeaderSystemEnv() } returns "localhost:${wireMockServer.port()}"
